...
def stack_last_n_obs_dict(all_obs, n_steps):
    ...
    result = {
        key: tf.zeros(
            shape=list(all_obs[-1][key].shape)[0:1] + [n_steps] + list(all_obs[-1][key].shape)[1:],
    ...
    for key in all_obs[-1]:
        result[key][:, start_idx:] = tf.concat(
...
class FurnitureRLSimEnvMultiStepWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        self.normalizer.load_state_dict(
            tf.saved_model.load(normalization_path)
        )
    ...
    def reset(
        ...
        self.best_reward = tf.zeros([self.env.num_envs], dtype=tf.float32)
        self.done = list()
        return {"state": nobs}
    ...
    def step(self, action: np.ndarray):
        ...
        if self.sparse_reward:
            reward = sparse_reward.numpy()
        else:
            reward = dense_reward.numpy()

        # Only mark the environment as done if it times out, ignore done from inner steps
        truncated = self.env.env_steps >= self.max_episode_steps

        nobs: np.ndarray = self.process_obs(obs)
        truncated: np.ndarray = truncated.numpy().squeeze()

        return {"state": nobs}, reward, truncated, truncated, info

    def _inner_step(self, action_chunk: tf.Tensor):
        dense_reward = tf.zeros([action_chunk.shape[0]], dtype=tf.float32)
        sparse_reward = tf.zeros([action_chunk.shape[0]], dtype=tf.float32)
        for i in range(self.n_action_steps):
            # The dimensions of the action_chunk are (num_envs, chunk_size, action_dim)
            obs, reward, done, info = self.env.step(action_chunk[:, i, :].numpy())
            self.obs.append(obs)

            # track raw reward
            sparse_reward += tf.squeeze(reward)

            # track best reward --- reward nonzero only one part is assembled
            self.best_reward += tf.squeeze(reward)
    ...
    def process_obs(self, obs: dict) -> np.ndarray:
        # Convert the robot state to have 6D pose
        robot_state = obs["robot_state"]
        robot_state = proprioceptive_quat_to_6d_rotation(robot_state)

        parts_poses = obs["parts_poses"]
        obs = tf.concat([robot_state, parts_poses], axis=-1)

        nobs = self.normalizer(obs, "observations", forward=True)
        nobs = tf.clip_by_value(nobs, -5, 5)
        return nobs.numpy()  # (n_envs, n_obs_steps, obs_dim)
